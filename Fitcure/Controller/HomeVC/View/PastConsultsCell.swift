...
class PastConsultsCell: UITableViewCell {
    ...
    var onDeletePressed: ((PatientRequestModel?, Int?)->())?
    var onAcceptPressed: ((PatientRequestModel?, Int?)->())?
    ...
    // MARK:- Custom Methods
    func setData(_ type: HomeVC.HomeSelectionTab, _ obj: PatientRequestModel, index: Int) {
        self.obj = obj
        self.index = index
    ...
    @IBAction private func btnAcceptPressed() {
        onAcceptPressed?(obj, index)
    }
    @IBAction private func btnRejectPressed() {
        onDeletePressed?(obj, index)
    }
